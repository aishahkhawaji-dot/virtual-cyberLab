...
def load_progress(student_id):
    ...
    score_weights = {
        ...
        "m9_detect_awarded":25, "m9_contain_awarded":25, "m9_preserve_awarded":25, "m9_escalate_awarded":25,
        "m10_report_awarded":25, "m10_lock_awarded":25, "m10_accounts_awarded":25, "m10_communication_awarded":25,
        "m11_scope_awarded":25, "m11_contain_awarded":25, "m11_follow_awarded":25, "m11_notify_awarded":25,
        "m12_detect_awarded":25, "m12_contain_awarded":25, "m12_preserve_awarded":25, "m12_recover_awarded":25,
    ...
    recalculated_score = max(step_score, completed_score)
...
def start_new_learner(name):
    ...
    if not learner_name:
        return None, "Please enter your name before creating a Student ID."
...
def _create_student_callback():
    name = str(st.session_state.get("new_learner_name", ""))
    st.session_state.student_login_error = ""
...
# ---------- STUDENT ACCESS ----------
    ...
    if not st.session_state.get("learner_id"):
        ...
        with a:
            ...
            st.text_input("Learner name *", key="new_learner_name", placeholder="Enter your name")
            name_ready = bool(str(st.session_state.get("new_learner_name", "")).strip())
            st.button("CREATE STUDENT ID", key="create_student_id", use_container_width=True, disabled=(not bool(SUPABASE_URL and SUPABASE_KEY)) or (not name_ready), on_click=_create_student_callback)
        with b:
    ...
    if not TEACHER_PASSWORD:
        ...
        if st.session_state.get("teacher_authenticated", False):
            ...
            if err:
                ...
                active_scores = [int(r.get("score",0) or 0) for r in rows if int(r.get("score",0) or 0) > 0]
                avg_score = round(sum(active_scores) / len(active_scores), 1) if active_scores else 0
                avg_progress = round(sum(len(r.get("completed") or []) for r in rows) / (total*16) * 100, 1) if total else 0
                st.markdown("""
                <style>
                [data-testid="stMetricValue"] { color:#2be0ec !important; font-weight:800 !important; }
                [data-testid="stMetricLabel"] { color:#d7e9f2 !important; font-weight:700 !important; }
                </style>
                ...
                display=[]
                for r in rows:
                    sid=r.get("student_id",""); name=r.get("learner_name",""); completed=len(r.get("completed") or [])
                    if q and q not in sid.lower() and q not in name.lower():
                        continue
                    score=int(r.get("score",0) or 0)
                    display.append({"Student ID":sid,"Name":name,"Score":f"{score}/1600","Progress":f"{completed}/16","Badge":badge_name(score),"Last Update":str(r.get("updated_at", ""))[:19].replace("T"," ")})
                if display:
                    # Responsive learner cards: compact on desktop and mobile-friendly on phones.
                    pending = st.session_state.get("teacher_delete_pending", "")
                    for r in rows:
                        sid = r.get("student_id", "")
                        name = r.get("learner_name", "")
                        if q and q not in sid.lower() and q not in name.lower():
                            continue
                        score = int(r.get("score", 0) or 0)
                        ...
                        with st.container(border=True):
                            st.html(f"""
                            <style>
                            .teacher-learner-card {{
                                display:grid;
                                grid-template-columns:1.35fr 1.25fr .9fr .95fr 1.5fr;
                                gap:10px;
                                align-items:center;
                                margin:2px 0 10px;
                            }}
                            .teacher-learner-card .field {{
                                min-width:0;
                            }}
                            .teacher-learner-card .label {{
                                color:#8da5b5;
                                font-size:11px;
                                font-weight:700;
                                margin-bottom:4px;
                            ...
                            @media (max-width: 768px) {{
                                ...
                                .teacher-learner-card .value {{ font-size:14px; }}
                            }}
                            </style>
                            <div class="teacher-learner-card">
                                <div class="field"><div class="label">Student ID</div><div class="value">{sid}</div></div>
                                <div class="field"><div class="label">Name</div><div class="value">{name}</div></div>
                                <div class="field"><div class="label">Score</div><div class="value">{score}/1600</div></div>
                                <div class="field"><div class="label">Progress</div><div class="value">{completed}/16</div></div>
                                <div class="field"><div class="label">Last Update</div><div class="value">{updated}</div></div>
                            </div>
                            """)
                            ra, da = st.columns(2)
                            pdf = build_progress_pdf_for_row(r)
                            ra.download_button("📄 REPORT", data=pdf or b"", file_name=f"Virtual_CyberLab_{sid}_Report.pdf", mime="application/pdf", key=f"teacher_report_{sid}", disabled=pdf is None, use_container_width=True)
                            if pending == sid:
                                if da.button("✓ CONFIRM", key=f"confirm_delete_{sid}", use_container_width=True):
                                ...
                                if da.button("🗑️ DELETE", key=f"delete_{sid}", use_container_width=True):
