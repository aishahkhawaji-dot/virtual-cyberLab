...
.st-key-teacher_mobile_table [data-testid="stVerticalBlock"]{gap:.25rem!important}
@media (max-width:600px){
  .st-key-teacher_desktop_table{display:none!important}
  .st-key-teacher_mobile_table{display:block!important}
  .st-key-teacher_mobile_table [data-testid="stHorizontalBlock"]{align-items:center!important;gap:.35rem!important}
  .st-key-teacher_mobile_table .stButton>button,.st-key-teacher_mobile_table [data-testid="stDownloadButton"] button{font-size:9px!important;min-height:32px!important;padding:2px 4px!important}
...
# ---------- STUDENT ACCESS ----------
    ...
    if not st.session_state.get("learner_id"):
        ...
        with a:
            ...
            st.caption("Name is required to create your Student ID.")
                    ...
                    with st.container(key="teacher_mobile_table"):
                        ...
                        for r in filtered_rows:
                            ...
                            with st.container(key=f"mobile_learner_{sid}"):
                                c1, c2, c3 = st.columns([1.55, 0.85, 1.35])
                                with c1:
                                    st.markdown(f"**{name}**")
                                    st.caption(sid)
                                with c2:
