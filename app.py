...
def load_progress(student_id):
    ...
    if err or not rows:
        return None, err or "Student ID (Use your own code) not found."
    row = rows[0]
    ...
    score_weights = {
        ...
        "m4_flags_awarded":40, "m4_risk_awarded":20, "m4_response_awarded":30, "m4_consequence_awarded":10,
...
def reconcile_mission_scores():
    # A mission is completed only when all of its scoring steps total 100.
    weights = {
        "m1": {"m1_sender_awarded":20,"m1_clues_awarded":40,"m1_decision_awarded":25,"m1_consequence_awarded":15},
        "m2": {"m2_findings_awarded":40,"m2_credential_awarded":30,"m2_action_awarded":30},
        "m3": {"m3_findings_awarded":40,"m3_assessment_awarded":20,"m3_action_awarded":30,"m3_consequence_awarded":10},
        "m4": {"m4_flags_awarded":40,"m4_risk_awarded":20,"m4_response_awarded":30,"m4_consequence_awarded":10},
        "m5": {"m5_findings_awarded":30,"m5_priority_awarded":25,"m5_actions_awarded":30,"m5_consequence_awarded":15},
        "m6": {"m6_obs_awarded":25,"m6_immediate_awarded":30,"m6_next_awarded":30,"m6_lesson_awarded":15},
        "m7": {"m7_ind_awarded":25,"m7_contain_awarded":30,"m7_recovery_awarded":25,"m7_evidence_awarded":20},
        "m8": {"m8_assess_awarded":25,"m8_contain_awarded":25,"m8_verify_awarded":25,"m8_recover_awarded":25},
        "m9": {"m9_assess_awarded":25,"m9_contain_awarded":25,"m9_investigate_awarded":25,"m9_communicate_awarded":25},
        "m10": {"m10_triage_awarded":25,"m10_contain_awarded":25,"m10_evidence_awarded":25,"m10_escalate_awarded":25},
        "m11": {"m11_report_awarded":25,"m11_protect_awarded":25,"m11_assess_awarded":25,"m11_communicate_awarded":25},
        "m12": {"m12_detect_awarded":25,"m12_contain_awarded":25,"m12_evidence_awarded":25,"m12_escalate_awarded":25},
        "m13": {"m13_scope_awarded":25,"m13_contain_awarded":30,"m13_follow_awarded":30,"m13_consequence_awarded":15},
        "m14": {"m14_chain_awarded":25,"m14_priority_awarded":25,"m14_actions_awarded":20,"m14_reason_awarded":15,"m14_communication_awarded":15},
        "m15": {"m15_timeline_awarded":25,"m15_integrity_awarded":25,"m15_analysis_awarded":25,"m15_communication_awarded":25},
        "m16": {"m16_detect_awarded":25,"m16_contain_awarded":25,"m16_investigate_awarded":25,"m16_impact_awarded":25},
    }
    old_completed=set(st.session_state.get("completed",set()))
    new_completed=set()
    total=0
    for mission, steps in weights.items():
        mission_score=sum(p for flag,p in steps.items() if st.session_state.get(flag,False))
        if mission_score == 100:
            new_completed.add(mission)
            total += 100
        else:
            total += mission_score
    if new_completed != old_completed:
        st.session_state.completed=new_completed
        st.session_state.progress_dirty=True
    if st.session_state.get("score",0) != total:
...
MISSIONS = {
    ...
    "m13": "Data Leakage Investigation", "m14": "Cyber Defender Mission",
...
# ---------- M5 ----------
    ...
    st.markdown("### STEP 1 — Identify the security risks.")
    findings = st.multiselect("Select the security risks.",[
    ...
    st.markdown("### STEP 2 — What should be prioritized?")
    priority = st.radio("What should be prioritized?",[
    ...
    st.markdown("### STEP 3 — Select the appropriate protective actions.")
    actions = st.multiselect("Select the appropriate protective actions.",[
    ...
    st.markdown("### STEP 4 — What is the safest lesson from this case?")
    consequence = st.radio("What is the safest lesson from this case?",[
...
# ---------- M6 ----------
    ...
    st.markdown("### STEP 1 — What does the evidence suggest?")
    observations=st.multiselect("What does the evidence suggest?",[
    ...
    st.markdown("### STEP 2 — Select the immediate safe actions.")
    immediate=st.multiselect("Select the immediate safe actions.",[
    ...
    st.markdown("### STEP 3 — What should happen next?")
    next_step=st.radio("What should happen next?",[
    ...
    st.markdown("### STEP 4 — Which security principle applies?")
    lesson=st.radio("Which security principle applies?",[
...
# ---------- M7 ----------
    ...
    st.markdown("### STEP 1 — Identify signs of account compromise.")
    indicators=st.multiselect("Identify signs of account compromise.",[
    ...
    st.markdown("### STEP 2 — Select appropriate containment actions.")
    containment=st.multiselect("Select appropriate containment actions.",[
    ...
    st.markdown("### STEP 3 — After securing recovery access, what should the user do?")
    recovery=st.radio("After securing recovery access, what should the user do?",[
    ...
    st.markdown("### STEP 4 — Which evidence should be preserved when available?")
    evidence=st.multiselect("Which evidence should be preserved when available?",[
...
# ---------- M8 ----------
    ...
    if st.button("SUBMIT INVESTIGATION",key="m8_submit"):
        ...
        if set(risks)==correct_risks and not st.session_state.m8_assess_awarded:
            complete("m8",25); st.session_state.m8_assess_awarded=True; awarded+=25
        ...
        if contain.startswith("Change the reused") and not st.session_state.m8_contain_awarded:
            complete("m8",25); st.session_state.m8_contain_awarded=True; awarded+=25
        ...
        if set(verify)==correct_verify and not st.session_state.m8_verify_awarded:
            complete("m8",25); st.session_state.m8_verify_awarded=True; awarded+=25
        ...
        if recovery.startswith("Create a unique") and not st.session_state.m8_recover_awarded:
            complete("m8",25); st.session_state.m8_recover_awarded=True; awarded+=25
        ...
        if all(st.session_state.get(x) for x in ["m8_assess_awarded","m8_contain_awarded","m8_verify_awarded","m8_recover_awarded"]):
            if "m8" not in st.session_state.completed:
...
# ---------- M13 ----------
    ...
    st.markdown("### STEP 1 — Identify the security concerns.")
    m13_scope=st.multiselect("Identify the security concerns.",["The folder uses public link access","The files contain student project information","The exposure duration is known to be several hours","The access history is currently unknown","The folder is automatically safe because it is inside the school drive","No investigation is needed if nobody reports misuse"],key="m13_scope")
    st.markdown("### STEP 2 — What should happen first?")
    m13_contain=st.radio("What should happen first?",["Ignore it until someone reports a problem.","Preserve relevant information, restrict inappropriate access, and notify the responsible team.","Delete the entire folder immediately.","Share the link with more users to test it."],key="m13_contain")
    st.markdown("### STEP 3 — Which follow-up actions are appropriate?")
    m13_follow=st.multiselect("Which follow-up actions are appropriate?",["Review sharing/access logs if available","Document what was discovered and when","Restore the intended access settings","Publicly name suspected users","Assess whether affected users need notification"],key="m13_follow")
    st.markdown("### STEP 4 — What is the main reason to preserve evidence before making unnecessary changes?")
    m13_consequence=st.radio("What is the main reason to preserve evidence before making unnecessary changes?",["Evidence can help determine scope and support the response.","Evidence should always be shared publicly.","Evidence is only useful if the attacker is known.","Evidence should be deleted to reduce storage."],key="m13_consequence")
...
# ---------- M14 ----------
    ...
    m14_chain=st.multiselect("Build the evidence chain.",["Unexpected MFA prompt","Suspicious link sent from the account","Unfamiliar device login","User denies initiating the activity","The event happened during school hours"],key="m14_chain")
    st.markdown("### STEP 2 — What is your first priority?")
    m14_priority=st.radio("What is your first priority?",["Ask the user to approve the next MFA prompt.","Contain and secure the account through the approved process while preserving evidence.","Delete the suspicious email and close the case.","Send the suspicious link to other users for testing."],key="m14_priority")
    st.markdown("### STEP 3 — After containment, select the appropriate actions.")
    m14_actions=st.multiselect("After containment, select the appropriate actions.",["Document relevant evidence and timestamps","Review account activity and authentication events","Escalate through the appropriate school/security process","Publish the incident details publicly","Delete all logs to protect privacy"],key="m14_actions")
    st.markdown("### STEP 4 — Why should the events be considered together?")
    m14_reason=st.radio("Why should the events be considered together?",["Multiple independent signals can indicate a connected account-compromise incident.","MFA makes every other security signal irrelevant.","A suspicious link is harmless when it comes from a known account.","Only the device login matters."],key="m14_reason")
    st.markdown("### STEP 5 — What is the safest communication approach?")
    m14_communication=st.radio("What is the safest communication approach?",["Use approved channels and share only the information needed by responsible people.","Forward the suspicious link to everyone so they can inspect it.","Post the user's account details publicly.","Ask students to investigate the account themselves."],key="m14_communication")
