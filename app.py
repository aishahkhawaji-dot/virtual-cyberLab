...
if st.session_state.page == "home":
    ...
    <div class="topbar">
      <div class="logo-wrap">
        ...
        <div><div class="logo-title">Virtual <span class="cyan">Cyber</span><span class="purple">Lab</span></div><div class="logo-sub">VIRTUAL CYBERSECURITY LABORATORY</div></div>
      </div>
      <div style="display:flex;align-items:center;gap:18px;justify-content:flex-end;flex:1;">
        <div class="safe"><b>Learn • Investigate • Protect</b><br>A safer digital tomorrow starts with you</div>
    ...
    <div class="hero">
      ...
      <div class="hero-title"><span class="cyan">Virtual</span> <span class="cyan">Cyber</span><span class="purple">Lab</span></div>
      <div class="hero-desc">A hands-on virtual laboratory to build your cybersecurity awareness</div>
    </div>
    """))
    ...
    cols=st.columns(4)
    cards=[
      (cols[0],"🎣","Phishing Lab","مختبر التصيد الإلكتروني","Analyze suspicious emails, links, QR codes and social-engineering attempts.","phish","phishing"),
      (cols[1],"🔐","Password & Identity","مختبر كلمات المرور والهوية","Investigate weak credentials, MFA alerts and account compromise.","pass","identity"),
      (cols[2],"🚨","Incident Response","مختبر الاستجابة للحوادث","Respond to security alerts and make evidence-based decisions.","inc","incident"),
      (cols[3],"🏆","Cyber Challenge","التحدي السيبراني","Combine your skills in multi-step cybersecurity investigations.","final","final")]
    for col,icon,title,ar,desc,cl,page in cards:
        with col:
            st.html(f'<div class="lab {cl}"><div class="lab-icon">{icon}</div><h3>{title}</h3><div class="ar">{ar}</div><p>{desc}</p></div>')
    ...
    st.html(textwrap.dedent("""
    <div style="margin-top:8px;padding:18px 22px;border:1px solid #205d7d;border-radius:18px;background:linear-gradient(145deg,rgba(8,36,59,.78),rgba(3,18,31,.82));text-align:center;">
      <div style="font-size:14px;color:#9bdfe8;font-weight:700;">© 2026 Virtual CyberLab</div>
      <div style="margin-top:7px;font-size:13px;color:#e8f4fa;">Developed & Designed by <b style="color:#25d9ea;">Aishah Ali Khawaji</b></div>
      <div style="margin-top:4px;font-size:12px;color:#a9bdca;">Gifted Secondary School for Girls - Gizan &nbsp;•&nbsp; School Principal: Samia Hammadi</div>
      <div style="margin-top:8px;font-size:11px;color:#718b9d;">Educational knowledge product • All rights reserved</div>
    </div>
    """))
    st.html('<div class="footer"><div>🛡️ Virtual CyberLab</div><div>Cybersecurity starts with awareness and grows with practice.</div></div>')
...
# ---------- M2 ----------
    ...
    if st.button("SUBMIT MISSION",type="primary"):
        ...
        pts=(50 if set(findings)==correct else 0)+(50 if action.startswith("Use the official") else 0)
        if pts==100: complete("m2",100); st.success("MISSION COMPLETE — 100/100 🛡️"); st.info("A familiar brand name does not make a link trustworthy. Verify the destination independently.")
        else: st.warning(f"Score: {pts}/100. Re-check the sender, urgency and destination URL.")
    mission_feedback("m2")
    if st.button("← BACK TO PHISHING LAB"): go("phishing")
...
# ---------- M4 ----------
    ...
    if st.button("SUBMIT MISSION",type="primary"):
        ...
        pts=(50 if set(redflags)==correct else 0)+(50 if response.startswith("Verify") else 0)
        if pts==100: complete("m4",100); st.success("MISSION COMPLETE — 100/100 🛡️"); st.info("Social engineering often combines urgency, authority and isolation. Independent verification breaks the attacker's advantage.")
        else: st.warning(f"Score: {pts}/100. Look for pressure, secrecy and requests that bypass normal verification.")
    mission_feedback("m4")
    if st.button("← BACK TO PHISHING LAB"): go("phishing")
