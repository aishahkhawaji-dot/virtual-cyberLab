...
def start_new_learner(name):
    ...
    sid = _new_student_id()
    learner_name = name.strip() or "Learner"
    ...
    if not TEACHER_PASSWORD:
        ...
        if st.session_state.get("teacher_authenticated", False):
            ...
            if err:
                ...
                avg_score = round(sum(int(r.get("score",0) or 0) for r in rows) / total, 1) if total else 0
                ...
                if display:
                    # Inline actions: REPORT and DELETE stay on the same row as each learner.
